...
public class ProjectManagementesuAction {
	// 显示项目管理的界面
	...
	//显示项目管理的界面
	public String displayesuprojectmanagement() {
		System.out.println("start display esuprojectmanagement");
		...
		//查询出该负责人所管理的项目
		System.out.println("开始根据enterprisesubuser的id查询Post");
		List esupost=postManage.esuidtofindpostbypage(enterprisesubuser.getId(),pageNo);
		System.out.println("完成Post的查询");
		ActionContext.getContext().getSession().put("esupost",esupost);
		...
		for (int i = 0; i < esupost.size(); i++) {
			Post post = (Post) esupost.get(i);
		...
		postinfo.put("esupost_tag", listtag);
		...
		ActionContext.getContext().getSession().put("esuprotag",protag);
		
		return "success";
	}
	
	//修改项目信息
	public String updateesuproject() {
