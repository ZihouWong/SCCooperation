...
<body>
	...
	<form id="addpost-form" action="eaddproject.action" method="post"
	...
	<div class="panel-group" id="accordion">
		<%
			if (!listEpost.isEmpty()) {
				//Post epost = (Post) listEpost.get(0);
				//Enterprisesubuser enterprisesubuser = epost.getManager_id();

				List post_esu = (List) request.getAttribute("post_esu");
				List post_tag = (List) request.getAttribute("post_tag");
				//获取出各个项目的负责人信息和tag信息
				//Enterprisesubuser pesu=(Enterprisesubuser)post_esu.get(0);
				//Tag ptag=(Tag)post_tag.get(0);
		%>
		...
		<%-- <div class="panel panel-default">
			<div class="panel-heading">
				<h4 class="panel-title">
					<a data-toggle="collapse" data-parent="#accordion"
						href="#collapse0"> 点击我进行展开，再次点击我进行折叠。第 1 部分 </a>
				</h4>
			</div>
			<div id="collapse0" class="panel-collapse collapse">
				<div class="panel-body">
					...
					<!-- 按钮触发模态框 -->
					<button class="btn btn-primary btn-lg" data-toggle="modal"
						data-target="#project1_Modal">编辑</button>
					<form id="modifypost1-form" action="eupdateproject.action?pageNo=1"
						method="post" enctype="multipart/form-data">
						<!-- 模态框（Modal） -->
						<div class="modal fade" id="project1_Modal" tabindex="-1"
							role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
							<div class="modal-dialog">
								<div class="modal-content">
									<div class="modal-header">
									...
									<div class="modal-body">
										...
										<p class="main-font">
											项目负责人:<%=epost.getManager_id()%></p>
										<select name="enterprisesubuser_id" class="form-control">
											<%
												for (int i = 0; i < Esu.size(); i++) {
														enterprisesubuser = (Enterprisesubuser) Esu.get(i);
											%>
											<option value="<%=enterprisesubuser.getId()%>"><%=enterprisesubuser.getNickname()%></option>
											<%
												}
		...
		<%
			for (int i = 0; i < listEpost.size(); i++) {
					...
					Enterprisesubuser enterprisesubuser = epost.getManager_id();
					//获取出各个项目的负责人信息和tag信息
					Enterprisesubuser pesu = (Enterprisesubuser) post_esu.get(i);
					Tag ptag = (Tag) post_tag.get(i);
		%>
		...
		<div class="panel panel-default">
			...
			<div id="collapse<%=i + 1%>" class="panel-collapse collapse">
				<div class="panel-body">
					<p class="main-font"> 项目名称:<%=epost.getProjectname()%></p>
					<p class="main-font">
						简介:<%=epost.getSummary()%></p>
					<p class="main-font">
						工作地点:<%=epost.getPlace()%></p>
					...
					<form id="modifypost<%=i + 1%>-form" action="eupdateproject.action"
						method="post" enctype="multipart/form-data">
						<!-- 模态框（Modal） -->
